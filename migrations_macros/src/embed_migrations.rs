...
pub fn expand(path: String) -> proc_macro2::TokenStream {
    ...
    quote! {
        diesel_migrations_async::EmbeddedMigrations::new(&[#(#embeded_migrations,)*])
    }
}
...
fn migration_literal_from_path(path: &Path) -> proc_macro2::TokenStream {
    ...
    quote!(diesel_migrations_async::EmbeddedMigration::new(
        include_str!(#up_sql_path),
        #down_sql,
        diesel_migrations_async::EmbeddedName::new(#name),
        diesel_migrations_async::TomlMetadataWrapper::new(#run_in_transaction)
    ))
}
